...
**Last updated:** 2026-08-03, later the same evening (session 18 — Session paused here again, resuming another day. Attempted to broaden the Layer 5 competitor benchmark to products 002/003/007 per session 17's plan — found and fixed FOUR separate real bugs along the way (search-card like/view mislabeling + hydration race, a second hydration race in the per-video like-count fetch, missing category-relevance filtering, and a serious frame-IDENTITY bug where screenshotted frames sometimes belonged to a completely different, unrelated video). None of the three products' benchmarks are trustworthy yet — all need a clean re-run with the fully-fixed script plus mandatory visual frame verification before anything downstream touches them. Full detail in session 18 below.)
**Owner:** Lilach
...
**Phase:** Collector Enhancement → analyzed → Analyze QA tool built → Analyzer v3 spec'd → Layer 7 implemented+corrected → ffmpeg fixed → **Layer 3 run on all 16/16 CONFIRMED variants** → Layer 5 execution diagnosis implemented+validated on 008B → **Layer 5 competitor benchmark escalated to a full statistical, multi-competitor architecture and run for product 008 (4/5 target)** → **findings synthesized in `CREATIVE_GAP_ANALYSIS.md`.** `data/video_results.csv` holds correct, verified, complete data for all 16 variants across products 002/003/007/008 — unchanged by any Layer 3/5/7 work, which write only to `data/tiktok-analytics/product*/`. Collector QA and Analyze QA both pass cleanly for this scope. **PRODUCT 009 DECISION: PROCEED ✅** (see session 5), independently re-verified by `scripts/analyze_qa.py` (session 6) — session 5's pacing recommendation is known to be misdirected (sessions 8/9) and now has a much more precise, evidence-backed picture (sessions 10-15) than a single cause. **Per explicit instruction, `learning_report.json` is deliberately NOT updated yet — holding until Layers 7, 3, and 5 are all complete (across all variants/products) and reviewed together; `CREATIVE_GAP_ANALYSIS.md` is the interim synthesis for that review.** Layer 5 (execution + competitor benchmark) has only run on 008B / product 008 so far — not yet extended to the other 15 variants or 3 products. Layers 1, 2, 4, 6, 8, 9, 10 of Analyzer v3 remain spec-only, not implemented. Products 001/004/005 remain a separate, pre-existing, documented gap — out of scope, not blocking. **2026-08-03: architectural decision approved — the platform must support two operating modes, Historical Backfill Mode and Daily Incremental Production Mode (full permanent requirement in `TIKTOK_AGENT_PLAN.md` → "Operating Modes — Permanent Production Requirement"). Approved as architecture only — NOT implemented; no scripts or workflow changed.** Next approved technical direction, in order: (1) ~~fix the `--seed-urls` `like_count: None` ranking bug in `scripts/layer5_competitor_benchmark.py`~~ — **DONE, session 17, regression-tested.** (2) broaden the Layer 5 competitor benchmark to products 002, 003, and 007 — **not yet started, resume here this evening.** (3) review that expanded evidence before touching `learning_report.json` or the creative pipeline — before generalizing product 008's findings to the rest of the portfolio or generating Product 009.

**PAUSED 2026-08-03 (end of session 18) — resuming another day.** Next action on resume, in order:
1. **Fix the blob-fetch bug before doing anything else** — the ffmpeg-based frame-extraction rewrite (Bug 4's fix, below) is NOT yet working: confirmed via a clean isolated re-test on `@malcolm.gregory6`'s video that `video.readyState` correctly reaches 4 (playable, real duration) after the `.play()` trigger, but `fetch(video.currentSrc)` on the resulting `blob:` URL throws `TypeError: Failed to fetch` — a browser-level restriction (likely the blob being revoked by TikTok's own SPA routing, or a CSP restriction on `connect-src` for blob: URIs), not a timing race. This is why product 007's supposedly-fully-fixed re-run came back **0/5** — every single frame extraction failed at the download step, not because of the CAPTCHA it also happened to hit at `comp0` (that CAPTCHA did clear on its own after 300s per the log, and was a separate, secondary issue). Two candidate directions to try next: (a) capture the video bytes at the network layer instead of re-fetching the blob after the fact (e.g. Playwright's `page.route()`/CDP Network domain, intercepting the actual CDN response before the page turns it into a blob), or (b) draw decoded frames to an off-screen `<canvas>` via `drawImage(video, ...)` and export with `canvas.toDataURL()` — still a real-decoded-frame capture immune to the original screenshot race, without needing to move the bytes out of the browser at all.
2. **Re-run ALL THREE of products 002, 003, and 007 from scratch** with the fully-fixed `scripts/layer5_competitor_benchmark.py` (relevance keywords + a WORKING frame-extraction fix, once step 1 is resolved) — none of the three currently-saved `product{pid}_layer5_competitor_benchmark.json` files can be trusted as-is (see session 18 for exactly why each one is known-bad).
3. **Personally open and visually inspect every extracted frame** (not just captions, not just the script's own success message) to confirm the creator/content actually matches the intended URL before treating any benchmark as valid — this is now mandatory, not optional, given session 18's confirmed frame-identity bug.
4. Run `python scripts/layer5_benchmark_qa.py --product-id 002,003,007` (all 7 checks) as an independent second check.
5. Only once all three pass real visual verification: do the `qualitative_review` agent pass (product-reveal timing, motion type, native-feel, etc.) for all ~15 competitors — still all null right now, not started for any product.
6. Update `CREATIVE_GAP_ANALYSIS.md` with the cross-product synthesis.
7. Unchanged standing rule: review that synthesis before touching `learning_report.json`, generalizing product 008's findings to the rest of the portfolio, or generating Product 009.
8. **Commit today's script fixes to git** — `scripts/layer5_competitor_benchmark.py`, `scripts/test_layer5_competitor_benchmark.py`, and the new `scripts/layer5_benchmark_qa.py` are all uncommitted as of end of session 18 (only session 17's earlier fix, commit `26bc876`, was committed today).
