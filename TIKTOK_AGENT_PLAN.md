...
| `scripts/tiktok_session_login.py` | ✅ One-time interactive TikTok Studio login — saves `data/tiktok-session.json` |
| `scripts/tiktok_analytics_collect.py` | ✅ Automated analytics collector — see PROJECT_STATUS.md for full fix history (DOM text-label extraction + direct video-ID navigation, both 2026-07-02) |
| `scripts/tiktok_collect_qa.py` | ✅ Standalone Collector QA suite (5 checks) — run after the collector, before `/tiktok analyze` |
| `scripts/analyze_qa.py` | ✅ Standalone Analyze QA suite (6 checks, added 2026-07-02) — run after `/tiktok analyze`, before trusting its PROCEED/PAUSE/CHANGE STRATEGY decision |

This table was last synced with reality on 2026-07-02 — it previously only listed the two 2026-06-11 scripts and had silently gone stale as the analytics/QA scripts were built. See `PROJECT_STATUS.md` for full dated history of every fix; this table is a snapshot, not the source of truth.

Both `generate_assets.py`/`generate_videos.py` are operational. Full end-to-end `/tiktok` pipeline (Steps 0–12) tested successfully on 2026-06-11 (product 001 — Astronaut Galaxy Projector). 4/4 MP4 variants generated, 1080×1920 H.264, 15s, no audio.
...
**Standalone QA companions (run directly with `python`, not slash commands):** `scripts/tiktok_collect_qa.py` after `/tiktok collect`/the analytics collector, `scripts/analyze_qa.py` after `/tiktok analyze` — both added 2026-07-02, see `PROJECT_STATUS.md` for full detail.
...
*Updated: 2026-07-02 — Collector fully fixed (DOM text-label extraction for saves/shares/watch-time/retention; direct video-ID URL navigation replacing dependency on TikTok's "Your top posts" leaderboard, which turned out to only ever show the top ~10 posts, not every video). Historical backfill completed and verified for products 002/003/007/008 (16/16 rows). Collector QA (`scripts/tiktok_collect_qa.py`) fixed (a false-positive session check on a rotating anti-bot cookie) and now passes. `/tiktok analyze` run for the first time ever on complete data — PRODUCT 009 DECISION: PROCEED, Curiosity leads Variant A. New standalone `scripts/analyze_qa.py` built and validated (6 checks). Full session-by-session detail in `PROJECT_STATUS.md` — this file only tracks the summary, per this file's own "do not maintain a second copy" convention.*
*Updated: 2026-07-02 — Analyzer v3 (`ANALYZER_V3_SPEC.md`) partially implemented: Layer 7 (pacing/retention) and Layer 3 (hook diagnosis) both run across all 16/16 CONFIRMED variants; Layer 5 (video/creative execution + a statistical multi-competitor benchmark with manual-CAPTCHA-handling support) implemented and run on 008B / product 008 only, not yet the other 15 variants or 3 products. Cross-layer findings synthesized in new file `CREATIVE_GAP_ANALYSIS.md`: cause (c) catalog/marketplace feel rated LIKELY in 16 of 16 variants — the single most universal finding — with zero organic motion and zero human presence confirmed in all 16 of our openings vs. all 4 of product 008's benchmarked competitors. `learning_report.json` deliberately not yet updated — held per standing instruction pending review of the synthesis. Layers 1, 2, 4, 6, 8, 9, 10 remain spec-only.*
*Updated: 2026-08-10 (sessions 21-23) — Layer 5's competitor benchmark extended from product 008 only to all four products (002, 003, 007, 008), each independently visually verified for frame identity and category relevance (full-video spot checks, not just the opening) and passing `layer5_benchmark_qa.py` (0 FAIL each). **Permanent rule change: Layer 5 benchmarks by narrow product category, not exact SKU** — a competitor only needs `CATEGORY_RELEVANCE_VERIFIED` + `FRAME_IDENTITY_VERIFIED` + real performance evidence, not the identical AliExpress listing (full rule in `ANALYZER_V3_SPEC.md`'s Layer 5 section). The session-20 DINOv2/OpenCLIP Same-Product Visual Identity Gate is deferred, not required for this goal, and remains uninstalled. `CREATIVE_GAP_ANALYSIS.md` updated accordingly: findings 1-5 (zero motion, zero human presence, zero product-in-use, catalog/screenshot feel, no emotional context) are now confirmed on all 4 products (19 real competitors total), not inferred for 3 of them. One new finding added: a distinct second creative beat (comparison, multi-product showcase, or full reveal ending) correlates with the top-performing competitor in every product benchmarked. `learning_report.json` still deliberately not updated; Product 009 not yet generated. Layers 1, 2, 4, 6, 8, 9, 10 remain spec-only. Full session-by-session detail in `PROJECT_STATUS.md` sessions 21-23.*
*Updated: 2026-08-11 (sessions 24-25) — Corrected a checkpoint overclaim (session 23's doc-sync had rolled Product 008 into "confirmed on all 4 products" without ever re-running its Layer 5 benchmark under the current methodology), then re-ran Product 008 from scratch and reached `QA_PASSED_PRODUCT008` (session 24) — all four products (002/003/007/008) are now genuinely QA-verified under identical rigor. Session 25 then synthesized all 16 of our variants vs. 20 QA-verified competitors across all four products into a final cross-product diagnosis in `CREATIVE_GAP_ANALYSIS.md`, approved with two corrections: (1) Rule 2 ("avoid marketplace/catalog-style presentation," revised from the original narrow "no AliExpress screenshot" wording) promoted to `PERMANENT CREATIVE RULE` alongside Rules 1 and 3, since the revised wording has full 16/16-vs-0/20 evidence; (2) the combined Layer 3+5+7 diagnosis explicitly does not claim competitor retention was measured — only that competitor creative presentation differs from ours. Rule 4 (a meaningful second beat beyond a flat demo) stays `MODIFIED RULE`/non-permanent, evidence in 3 of 4 products. A new data-reliability finding: every "STRONG" retention result in the portfolio comes from a variant with under 10 views. `ANALYZER_V3_SPEC.md` updated with a permanent "CROSS-PRODUCT LEARNING ARCHITECTURE" section codifying this methodology for future products, not a one-off. `learning_report.json` and Product 009 still untouched as of session 25.*
*Updated: 2026-08-11 (session 26) — The approved Layer 3+5+7 synthesis above was incorporated into `learning_report.json`: a new `creative_rules` block records the 3 Permanent Creative Rules, Rule 4 as non-permanent (`do_not_promote_to_permanent: true`), the combined diagnosis with an explicit causality guardrail (no competitor retention curves, no causal proof, specific forbidden overclaims listed), the sub-10-view data-quality finding, and a `next_causal_test` scaffold (`test_status: NOT_YET_RUN`) so a future creative can serve as the causal test this synthesis says is still missing. No pre-existing field was deleted; `product_009_brief.first_frame_requirement` was flagged as superseded via a new compliance-note field rather than rewritten, since regenerating Product 009 guidance is out of scope. Re-validated with `scripts/analyze_qa.py` — still 6/6 PASS. Committed and pushed (commit `048542b`). Creative pipeline, Product 009, and Daily Incremental Mode remain untouched, pending human review of this `learning_report.json` update. Full session-by-session detail in `PROJECT_STATUS.md` sessions 24-26.*
